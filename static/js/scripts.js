...
document.addEventListener("click", function(event) {
    ...
    if (event.target.closest('#lowercase-letter-button')) {
        event.preventDefault();
        convertToLowercase();
    }
    if (event.target.closest('#uppercase-letter-button')) {
        event.preventDefault();
        convertToUppercase();
    }
    if (event.target.closest('#capitalize-button')) {
        event.preventDefault();
        capitalizeText();
    }
    if (event.target.closest('#alternate-button')) {
        event.preventDefault();
        alternateText();
    }
    if (event.target.closest('#camelcase-button')) {
        event.preventDefault();
        convertToCamelCase();
    }
...
// ==========================================================================
// 7. SISTEMA AUTOMÁTICO DE CARREGAMENTO DE CSS SOB DEMANDA (SPA)
// ==========================================================================
...
// ==========================================
// 8. CAPTURA DE EVENTOS TIPO INPUT (GLOBAL PARA SPA)
// ==========================================
...
// ==========================================================================
// 9. ORQUESTRAÇÃO DE INICIALIZAÇÃO DE MÓDULOS NA MUDANÇA DE ROTA (SPA)
// ==========================================================================
...
// Função pura para buscar e atualizar o contador
async function atualizarContadorAcessos() {
    const visitsElement = document.getElementById('visits');
    if (!visitsElement) return; // Se o elemento não estiver na tela atual (ex: outra rota), não faz nada
    try {
        // Exemplo usando uma API estável ou a sua lógica atual de fetch
        const response = await fetch('https://api.countapi.xyz/hit/dev-tool-box-spa/visitas');
        const data = await response.json();
        if (data && typeof data.value === 'number') {
            visitsElement.textContent = data.value.toLocaleString('pt-BR');
        }
    } catch (error) {
...
// Chamar a função quando a página carregar
document.addEventListener('DOMContentLoaded', () => {
    // Atualizar o ano no rodapé (aproveitando para manter seu código organizado)
    const yearElement = document.getElementById('current-year');
    if (yearElement) {
        yearElement.textContent = new Date().getFullYear();
    }
