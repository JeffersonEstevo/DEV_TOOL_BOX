...
// Garantia absoluta contra erro de re-declaração no escopo do navegador
window.PALETAS_PRESETS = window.PALETAS_PRESETS || [
    {
...
window.gerarTemplateCSS = function(hexPrimary, hexSecondary, hexBg, ffRegular, radius) {
...
window.atualizarCSSPreview = function() {
...
window.gerarSwatchCor = function(label, hex) {
    return `
...
window.renderizarPaletasPresets = function() {
    const container = document.getElementById('grid-paletas-presets');
    if (!container || !window.PALETAS_PRESETS) return;

    container.innerHTML = window.PALETAS_PRESETS.map((p) => `
        <div class="card-paleta" style="border: 1px solid var(--border-color); border-radius: var(--radius); padding: 1.2rem; background: var(--bg-main);">
            ...
            <div style="display: flex; gap: 0.5rem; margin: 0.4rem 0 1rem 0; align-items: center;">
                ${window.gerarSwatchCor("Primária", p.light.primary)}
                ${window.gerarSwatchCor("Secundária", p.light.secondary)}
                ${window.gerarSwatchCor("Fundo", p.light.bg)}
            </div>

            <span style="font-size: 0.75rem; font-weight: 600; color: var(--text-muted);">MODO ESCURO EQUIVALENTE</span>
            <div style="display: flex; gap: 0.5rem; margin: 0.4rem 0 1rem 0; align-items: center;">
                ${window.gerarSwatchCor("Primária", p.dark.primary)}
                ${window.gerarSwatchCor("Secundária", p.dark.secondary)}
                ${window.gerarSwatchCor("Fundo", p.dark.bg)}
            </div>

            <div style="display: flex; gap: 0.5rem; margin-top: 0.8rem; border-top: 1px dashed var(--border-color); padding-top: 0.8rem;">
                <button class="btn btn-secondary" style="flex: 1; font-size: 0.75rem; padding: 0.4rem;" onclick="window.aplicarPaletaNoGerador('${p.light.primary}', '${p.light.secondary}', '${p.light.bg}')">
                    <i class="bi bi-sun"></i> Usar Claro
                </button>
                <button class="btn btn-secondary" style="flex: 1; font-size: 0.75rem; padding: 0.4rem;" onclick="window.aplicarPaletaNoGerador('${p.dark.primary}', '${p.dark.secondary}', '${p.dark.bg}')">
...
window.aplicarPaletaNoGerador = function(primary, secondary, bg) {
    ...
    window.atualizarCSSPreview();
    window.alternarAbaCSS('aba-gerador');
};

window.alternarAbaCSS = function(idAba) {
...
// ==========================================
// DELEGAÇÃO DE EVENTOS DINÂMICOS
// ==========================================
if (!window.__cssModuleListenersAtivos) {
    window.__cssModuleListenersAtivos = true;

    document.addEventListener('input', function(e) {
        const target = e.target;
        if (!target) return;

        if (target.id === 'seletor-cor-primary') {
            const hex = document.getElementById('hex-primary');
            if (hex) hex.value = target.value;
            window.atualizarCSSPreview();
        } else if (target.id === 'hex-primary') {
            const pick = document.getElementById('seletor-cor-primary');
            if (pick && /^#[0-9A-F]{6}$/i.test(target.value)) pick.value = target.value;
            window.atualizarCSSPreview();
        } else if (target.id === 'seletor-cor-secondary') {
            const hex = document.getElementById('hex-secondary');
            if (hex) hex.value = target.value;
            window.atualizarCSSPreview();
        } else if (target.id === 'hex-secondary') {
            const pick = document.getElementById('seletor-cor-secondary');
            if (pick && /^#[0-9A-F]{6}$/i.test(target.value)) pick.value = target.value;
            window.atualizarCSSPreview();
        } else if (target.id === 'seletor-cor-bg') {
            const hex = document.getElementById('hex-bg');
            if (hex) hex.value = target.value;
            window.atualizarCSSPreview();
        } else if (target.id === 'hex-bg') {
            const pick = document.getElementById('seletor-cor-bg');
            if (pick && /^#[0-9A-F]{6}$/i.test(target.value)) pick.value = target.value;
            window.atualizarCSSPreview();
        }
    });
    ...
    document.addEventListener('change', function(e) {
        if (e.target && (e.target.id === 'seletor-ff-regular' || e.target.id === 'seletor-radius-css')) {
            window.atualizarCSSPreview();
        }
    });

    document.addEventListener('click', function(e) {
        const btn = e.target.closest('.btn-copiar-codigo-css');
        if (!btn) return;
...
// Inicializador Único
window.inicializarModuloCSS = function() {
    window.renderizarPaletasPresets();
    window.atualizarCSSPreview();
};

window.executarGatilhoCSS = function() {
    window.inicializarModuloCSS();
};

if (document.readyState === 'complete' || document.readyState === 'interactive') {
    window.executarGatilhoCSS();
} else {
    document.addEventListener('DOMContentLoaded', window.executarGatilhoCSS);
}

window.addEventListener('hashchange', function() {
    if (window.location.hash.includes('css')) {
        setTimeout(window.executarGatilhoCSS, 50);
    }
});