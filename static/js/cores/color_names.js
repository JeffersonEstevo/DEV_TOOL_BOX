...
function renderizarGridCores(filtro = "") {
    const gridContainer = document.getElementById('grid-nomes-cores');
    if (!gridContainer) return;

    gridContainer.innerHTML = '';
    const termo = filtro.toLowerCase().trim();

    LISTA_CORES_CSS.forEach(cor => {
        // Aplica o filtro caso exista digitação na busca
        if (termo && !cor.name.toLowerCase().includes(termo) && !cor.hex.toLowerCase().includes(termo)) {
            return;
        }
        ...
        preview.className = 'card-cor-preview';
        preview.style.backgroundColor = cor.hex;

        const info = document.createElement('div');
        info.className = 'card-cor-info';

        const titulo = document.createElement('span');
        titulo.className = 'card-cor-titulo';
        titulo.textContent = cor.name;

        const codigoHex = document.createElement('span');
        codigoHex.className = 'card-cor-hex';
        codigoHex.textContent = cor.hex.toUpperCase();
        ...
        card.addEventListener('click', () => {
            navigator.clipboard.writeText(cor.hex.toUpperCase()).then(() => {
                card.title = "Copiado!";
