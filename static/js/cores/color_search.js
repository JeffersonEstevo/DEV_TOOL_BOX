// ==========================================================================
// Banco de cores protegido contra re-declaração na SPA
// ==========================================================================
if (typeof window.bancoDeCoresCompleto === 'undefined') {
    window.bancoDeCoresCompleto = [];
}
...
// Cálculo matemático de Distância Real entre Cores
...
function processarBusca(termo) {
    if (window.bancoDeCoresCompleto.length === 0) return;
    ...
    if (ehHex || (termo.length >= 4 && /^#[0-9A-Fa-f]+$/.test(termo.startsWith('#') ? termo : '#' + termo))) {
        ...
        const listaComDistancia = window.bancoDeCoresCompleto.map(cor => {
        ...
        const resultadosNome = window.bancoDeCoresCompleto.filter(cor => 
...
function renderizarGridProximas(lista, mensagemTopo) {
    ...
    lista.forEach(cor => {
        ...
        card.addEventListener('click', () => {
            ...
            if (inputHex) inputHex.value = cor.color_name;
...
async function iniciarColorSearch() {
    ...
    // Se o banco estiver vazio, busca os dados de forma segura
    if (window.bancoDeCoresCompleto.length === 0) {
        try {
            const resposta = await fetch('cores.json');
            if (!resposta.ok) {
                throw new Error(`Arquivo JSON não encontrado (Status: ${resposta.status})`);
            }
            window.bancoDeCoresCompleto = await resposta.json();
        } catch (erro) {
...
// Execução segura imediata ao injetar o script na SPA
iniciarColorSearch();